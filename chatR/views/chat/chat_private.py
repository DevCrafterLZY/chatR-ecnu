...
from chatR.tools.utils import private_get_chat_history, baidu_translate, process_raw_docs
...
def private_pdf_path():
    ...
    filename = file[1]
    filename_without_extension = os.path.splitext(filename)[0]
    path = 'http://localhost:5000/static/pdf/' + str(i_id) + '/' + filename_without_extension + '/' + filename
...
async def private_send_message():
    ...
    chat = db.fetchone('select * from chat where c_id = %s', c_id)
    i_id = chat[1]
    history = private_get_chat_history(c_id, 1)
